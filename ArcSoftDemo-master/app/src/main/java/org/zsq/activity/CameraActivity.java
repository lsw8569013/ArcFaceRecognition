...
import com.arcsoft.library.FaceManager;
...
public class CameraActivity extends Activity implements CameraCallback {
    ...
    private boolean notDetection = true;
...
//            }
    ...
    public void onPreviewFrame(final byte[] bytes, final Camera camera) {
            ...
            start(bytes, mWidth, mHeight, orientation);
...
//        EventBus.getDefault().post(new FaceData(data, width, height, orientation));
        List<AFT_FSDKFace>  list  = faceService.detection(data, width, height, orientation);
        if(list.size()>0 ){
            if(notDetection){
                if(list.size() > 1){
                    Toast.makeText(this,"请不要在摄像头前面站多个人！",Toast.LENGTH_SHORT).show();
                    return;
                }
                notDetection = false;
                faceService.cameraRecognize(data, width, height,list.get(0),orientation);
                notDetection = true;
            }else{
                Log.e("lsw","正在--检索头像");
            }
    ...
    public void onMessageEvent(FaceResponse event) {
        if (event.getCode() != 0) {
            clearUI();
            Log.e("lsw", "错误码：" + event.getCode());
        } else {
//            Log.e("lsw", "成功：" + event.getType() );
            switch (event.getType()) {
                ...
                case MATCH: {
//                    notDetection = false;
                    Toast.makeText(this,"识别"+ event.getName() +" -- 时间--"+(System.currentTimeMillis() - mStartTime) ,Toast.LENGTH_LONG).show();
                    Log.e("lsw","识别耗时"+ (System.currentTimeMillis() - mStartTime)   );
                    Log.e("lsw", "match : " + event.getScore()  + "-- name"+event.getName());
                    faceOut((ArcsoftFace) event.getFace(), event.getScore(), event.getName(), event.getOrientation());
